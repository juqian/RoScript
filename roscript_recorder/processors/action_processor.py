...
def get_action_type(touch_fingertip_groups, pre_action_frame, keyboard_models):
    ...
    if touch_during_frame_count <= stay_frame_count_limit:
        if len(touch_fingertip_groups) > 2:
            return "swipe"

        if len(touch_fingertip_groups) == 2:
            first_deepest_touch_position = group_processor.get_deepest_from_sidelook(touch_fingertip_groups[0]).get_touch_position()
            second_deepest_touch_position = group_processor.get_deepest_from_sidelook(touch_fingertip_groups[1]).get_touch_position()
            distance_square = math.pow((first_deepest_touch_position[0]-second_deepest_touch_position[0]),2)+math.pow((first_deepest_touch_position[1]-second_deepest_touch_position[1]),2)
            # 运动距离较远，考虑到误差因素，推测是swipe操作
        ...
        if len(touch_fingertip_groups) == 1:
            ...
            end_touch_position = touch_fingertip_groups[0][-1].get_touch_position()
        ...
        if distance_square <= math.pow(pixel_move_distance,2):
            return "long_press"
        stay_time_touch_position = touch_fingertip_groups[0][stay_frame_count_limit].get_touch_position()
