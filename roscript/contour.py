...
@author: 29965
屏幕轮廓读写 
Screen Profiles Read and Write 
"""
...
def get_region_contour(region=[0, 0, 1, 1]):  # 获取目标区域坐标
    """获取设备屏幕轮廓在图像中的像素坐标
    Args:
...
def caculate_image_contour(img, is_first=True):
    ...
    canny = cv2.Canny(img, 50, 100)  # 使用canny算法得到轮廓图
...
def calculating_image_contour(snapscreen=Config.get_calibration_screenshot_image(),
                              isCutTwice=True, isAdvanced=False):  # 获取设备轮廓
    """get image contour"""
    try:
        open(snapscreen)
    except FileNotFoundError as e:
        raise FileNotFoundError(e)
    img = cv2.imread(snapscreen)
    if isAdvanced:
        _, device_contour = simple_detect(img)
        contour_map = advanced_detect(img, device_contour)
    else:
        contour_map = caculate_image_contour(img)  # 初步截图设备（带边框）轮廓
    x_left, y_top, x_right, y_down = contour_map
    img_contour = img[y_top:y_down, x_left:x_right]
    if isCutTwice:  # 判断并截取屏幕轮廓
        contour_map2 = caculate_image_contour(img_contour, is_first=False)
        ...
        contour_map = [new_x_left, new_y_top, new_x_right, new_y_down]
        img_contour = img[new_y_top:new_y_down, new_x_left:new_x_right]
    #        cv2.rectangle(img,(new_x_left,new_y_top),(new_x_right,new_y_down),(0,0,255), 2)
    cv2.imwrite(Config.get_devicescreen_image(), img_contour)
    Config.record_calibration_file(contour_map)
