...
RoScriptHome = CODE_BASE[:CODE_BASE.rfind("RoScript") + len("RoScript")]

if USE_PYTHON_PROJECT_AS_HOME:
...
def __read_config_file(config_file_path):
    file = open(path.join(g_tool_home, config_file_path))
    file_content = yaml.load(file, Loader=yaml.Loader)
    file.close()
    return file_content

# config文件参数
g_config_content = __read_config_file(g_config_path)
# data文件参数
g_sut_content = __read_config_file(g_sut_path)
...
class Config(object):
    ...
    def get_calibration_camera_file_path():
        return path.join(g_tool_home, g_camera_data_path)
    ...
    @staticmethod  
    # 获取起点规避距离
    def get_origin_detour_length():# 获取图片分辨率
    ...
    def update_focal_length(focus=73):# 更新相机焦距
        file = open(path.join(g_tool_home, g_sut_path), 'w')
    ...
    def update_circle_to_pen(circle_to_pen_distance):
        """更新笔尖到标志圆心的距离"""
        file = open(path.join(g_tool_home, g_config_path), 'w')
