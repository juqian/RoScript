...
class Screenshot(object):
    ...
    def __init__(self,
        ...
        if screen_shot_index != 0:
            self.screen_shot_path = self.__get_screen_shot_image(screen_shot_index,
                                                            virtual_debug)
    ...
    def __get_widget_path(self, widget_image):
        ...
        self.__match_exist_image(widget_dir, widget_image)
        widget_path = '{}/{}'.format(widget_dir, widget_image)
    ...
    def __scale_normalize(self, widget_path, widget_image):# 大小修改
        ...
        scaling_widget = origin_widget.resize((int(width * scaling), int(height * scaling)),
                                              Image.ANTIALIAS)
        #将缩放后的控件存为临时控件
        scaling_widget_path = '{}/_temporary.png'.format(TestScript.script_widgets_dir())
        scaling_widget.save(scaling_widget_path)
        return scaling_widget_path
    
    # 检测图片是否存在
    def __match_exist_image(self, dir_path, image_file):
        """检测文件是否存在
        Args:
            dir_path: 文件夹路径
            image_file: 图像文件名
        """
        try:
            open('{}/{}'.format(dir_path, image_file))
    ...
    #当前屏幕图片
    def __get_screen_shot_image(self, screen_shot_index, virtual_debug):
        # 基图片
        if virtual_debug:
            screen_shot_dir = TestScript.get_virtual_debug_dir()
        else:
            screen_shot_dir = TestScript.script_temporary_dir()
        screen_shot_name = '{}_{}.png'.format(TestScript.get_pyname(),
                                              screen_shot_index)
        self.__match_exist_image(screen_shot_dir, screen_shot_name)
        return '{}/{}'.format(screen_shot_dir, screen_shot_name)
    ...
    def get_image_match_result(self, algorithm='tcfn'):
        ...
        cv2.rectangle(device_screen_image,(int(x), int(y)),
                      (int(x + w), int(y + h)),
                      (0,0,255),2)
        match_result_path = '{}/{}_end.png'.format(TestScript.script_test_dir(),
                                                   filename)
    ...
    def match(self, device_image_path, algorithm):
        ...
        #选择算法
        match_result = template_match.TEMPLATE_MATCHERS[algorithm](device_image_path,
                                                           self.widget_image_path)
...
def crop_snap_screen(snap_path):
    ...
    device_image.save(snap_path)# 存储snapscreen