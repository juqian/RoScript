...
class RecordVideo(object):
    ...
    def record_video(self, video_dir):
        ...
        cameraCapture = cv2.VideoCapture(Config.get_capture_id() + cv2.CAP_DSHOW)  # 微软特有
    ...
    @staticmethod
    def monitor_pen_fall(start_fall_img_idx):
        """ 存储落笔过程中辅助摄像头拍摄的图像"""
...
class SutMonitor(object):
    ...
    def get_img(save_path, size=Config.get_screenshot_size()):
        ...
        if ret:
            ...
            print('Cannot Get the Image of Robot Marking')
        camera_capture.release()
...
class PenMonitor(object):
    ...
    def collect_pen_fall_images(pen_current_height,
        ...
        g_run_event = True
        t = threading.Thread(target=RecordVideo.monitor_pen_fall, args=(start_fall_img_idx,))
        t.setDaemon(True)  # 将线程设置为后台线程
        # 开启实时采集线程
        t.start()
