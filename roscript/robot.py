...
class Robot(object):
    ...
    def get_photo_index(self, command, save_frame=False):
        ...
        # 记录拍照时间
        self.log.record_TP_result()
        return self.temp_image_index

    # 滑动操作起始位置相对距离
    def __swipe_distance(self, region_contour, region_size, dx, dy):
        """ 屏幕中心点到swipe起点的距离
    ...
    def swipe(self, region, direction):
        ...
        if direction == 'up':  # 向上滑动
            # 计算滑动操作起点坐标
            px, py = self.__swipe_distance(region_contour, region_size, 0, -region_size[1])
            ...
            # 计算滑动操作起点坐标
            px, py = self.__swipe_distance(region_contour, region_size, -region_size[0], 0)
            swipe_start_coor = normalize_device_direction(px, py)  # 将图像坐标系转换成机械臂坐标系
