...
class Rcs(object):
    ...
    def __init__(self):
        """数据初始化."""
        self.virtual_debug = Config.get_virtual_debug_model()  # 虚拟调试模式 写到config,屏蔽机器未启动
    ...
    def find(self, widget_image_name, region=None):
        ...
        # 查找控件坐标
        # 分步规避动作
        self.log.record_action_start("find",
    ...
    def match(self, widget_image_name, region=None):
        ...
        self.log.record_action_start("match",
                                     "'{}'".format(widget_image_name))
        # 匹配结果
        ...
        if not match_widget_exist:
            match_widget_exist = self.__check_widget_exist(widget_image_name,
                                                           region)
        ...
        if self.virtual_debug:
            print("[Virtual Debug] match('{}'): Succeed!".format(widget_image_name))
    ...
    def click(self, target, region=None):
        ...
        if not result:
            if self.virtual_debug:
                print("[Virtual Debug] click('{}'): Failed!".format(target))
                self.log.record_action_end()
                return result
            else:
            ...
            if self.virtual_debug:
                print("[Virtual Debug] click('{}'): Succeed!".format(target))
                # action_exec_time = self.robot_dev.get_action_exec_time()
        self.log.record_action_end()
    ...
    def double_click(self, widget_image_name, region=[0, 0, 1, 1]):
        ...
        Raises: 
            Exception: 没有找到指定控件
        """
        self.log.record_action_start("double click",
                                     "'{}'".format(widget_image_name))
        ...
        if self.lasted_widget == widget_image_name:
            ...
            # action_exec_time = self.robot_dev.get_action_exec_time()
            if self.virtual_debug:
                print("[Virtual Debug] double click('{}'): "
                      "Succeed!".format(widget_image_name))
            self.log.record_action_end()
            return True
        ...
        if self.__check_widget_exist(widget_image_name, region):
            ...
            if self.virtual_debug:
                print("[Virtual Debug] double click('{}'): "
                      "Failed!".format(widget_image_name))
                self.log.record_action_end()
                return
            else:
                self.log.record_exception()
    ...
    def press_keyboard(self, keyboard_name, text):
        ...
        Raises: 
            Exception: 没有找到指定键盘
        """
        self.log.record_action_start("press keyboard",
                                     "'{}', '{}'".format(keyboard_name, text))
        ...
        keyboard_file = Keyboard(self.log, keyboard_name)  # 获取指定键盘信息
        ensure_keyboard_result = keyboard_file.get_keyboard_match_result(
            self.screen_shot_index, self.virtual_debug)
        ...
        if (x <= 0 or y <= 0) or (x + w >= screen_size[0] or
                                  y + h >= screen_size[1]):
            # 没有匹配到键盘图像模型,或者匹配到的键盘模型在边界上
            if self.virtual_debug:
                print("[Virtual Debug] press keyboard('{}', '{}'): "
                      "Failed!".format(keyboard_name, text))
                self.log.record_action_end()
                return
            else:
                self.log.record_exception()
        ...
        key_esc = False  # 是否转义
        # 按顺序键入按键
        # 使用“[”和"]"作为转义字符,存贮特殊按键,比如"Enter"
        for key in text:
            if key_esc:
                if key == ']' and key_str != '':
                    key_esc = False
                    self.__click_key(keyboard_file,
                                     [x, y, w, h],
                                     key_str)
                    key_str = ''
                else:
                ...
                key_str += key
                self.__click_key(keyboard_file, [x, y, w, h], key_str)
                key_str = ''
        #        action_exec_time = self.robot_dev.get_action_exec_time()
        if self.virtual_debug:
            print("[Virtual Debug] press keyboard('{}', '{}'): "
                  "Succeed!".format(keyboard_name, text))
    ...
    def __click_key(self, keyboard_file, keyboard_area, text):
        ...
        """
        # 获取键位坐标和键盘识别结果
        key_x, key_y = keyboard_file.get_key_match_results(
            text, keyboard_area)
    ...
    def swipe(self, direction, region=[0, 0, 1, 1]):
        ...
        Args:
            ...
            region: 控件查询的范围,默认为整个被测设备屏幕
        """
        self.log.record_action_start("swipe",
                                     "'{}'".format(direction))
        self.detour('swipe', region=region)
        self.__get_snapscreen('swipe')
        reg = self.__is_region(region)
        self.robot_dev.swipe(reg, direction)

        #        action_exec_time = self.robot_dev.get_action_exec_time()
        if self.virtual_debug:
            print("[Virtual Debug] swipe('{}'): Succeed!".format(direction))

        self.log.record_action_end()

    def long_press(self, widget_image_name,
                   region=[0, 0, 1, 1]):
        """在指定控件位置长按
        ...
        Raises: 
            Exception: 没有找到指定控件
        """
        self.log.record_action_start("long press",
                                     "'{}'".format(widget_image_name))
        # 规避动作
        ...
        if self.__check_widget_exist(widget_image_name, region):
            ...
            self.robot_dev.long_press(x, y)
            if self.virtual_debug:
                print("long press('{}') Succeed!".format(widget_image_name))
        else:
            # 未能识别成功            
            if self.virtual_debug:
                print("[Virtual Debug] long press('{}'): "
                      "Failed!".format(widget_image_name))
            else:
                self.log.record_exception()
                self.reset_arms()
    ...
    def move_to(self, *args):
        ...
        Raises: 
            Exception: 没有找到指定控件
        """
        self.log.record_action_start("move to",
                                     "'{}'".format(args[0]))
        self.detour('move to')
        ...
        if len(args) == 1 and self.__is_image_file(args[0]):
            ...
            if self.__check_widget_exist(args[0]):
                x, y = self.match_result.get_coordinates()
            else:
                if self.virtual_debug:
                    print("[Virtual Debug] move_to('{}'): "
                          "Failed!".format(args[0]))
                    self.log.record_action_end()
                    return
                else:
                    self.log.record_exception()
            ...
            if not self.virtual_debug:
                print("[Virtual Debug] move_to('{}'): Failed!".format(args[0]))
            else:
                self.log.record_exception()
                self.reset_arms()
                raise Exception("move_to('{}') Error!".format(args[0]))
        ...
        #        action_exec_time = self.robot_dev.get_action_exec_time()
        if self.virtual_debug:
            print("[Virtual Debug] move_to('{}'): Succeed!".format(args[0]))
    ...
    def pen_down(self):
        ...
        #        action_exec_time = self.robot_dev.get_action_exec_time()
        if self.virtual_debug:
            print('[Virtual Debug] Pen down Ends.')
    ...
    def pen_up(self):
        ...
        #        action_exec_time = self.robot_dev.get_action_exec_time()
        if self.virtual_debug:
            print('[Virtual Debug] Pen up Ends.')
    ...
    def move_outside_of_screen(self):
        ...
        #        action_exec_time = self.robot_dev.get_action_exec_time()
        if self.virtual_debug:
            print('[Virtual Debug] Move outside of screen Ends.')
        self.log.record_action_end()
    ...
    def drag(self, start_widget, end_widget,
        ...
        Raises: 
            Exception: 没有找到指定控件
        """
        self.log.record_action_start("drag",
                                     "'{}', '{}'".format(start_widget, end_widget))
        ...
        if self.lasted_widget == start_widget:
            ...
            if self.__check_widget_exist(end_widget, region2):
                ...
                if self.virtual_debug:
                    print("[Virtual Debug] drag('{}', '{}'): Failed!".format(
    ...
    def assert_exist(self, widget_image_name, region=[0, 0, 1, 1]):
        ...
        if self.algorithm.match_result_is_right(self.match_result.get_similarity()):
            ...
            if self.virtual_debug:
                print("[Virtual Debug] assert_exist('{}'): Yes!".format(widget_image_name))
            else:
            ...
            if self.virtual_debug:
                print("[Virtual Debug] assert_exist('{}'): No!".format(widget_image_name))
            else:
    ...
    def assert_not_exist(self, widget_image_name, region=[0, 0, 1, 1]):
        ...
        if not self.algorithm.match_result_is_right(self.match_result.get_similarity()):
            self.log.record_assert_match_result("Yes")
            if self.virtual_debug:
                print("[Virtual Debug] assert_not_exist('{}'): Yes!".format(widget_image_name))
            else:
            ...
            self.log.record_assert_match_result("No")
            if self.virtual_debug:
                print("[Virtual Debug] assert_not_exist('{}'): No!".format(widget_image_name))
            else:
    ...
    def reset_arms(self):
        ...
        self.__get_snapscreen('reset')
        # 控制机器和程序结束运行
        # self.end_script_run()  # TODO 将里面的release放在exit钩子里
        ...
        self.log.record_action_end()
        # 记录脚本执行结束
        # self.log.record_script_end()  # TODO 放在exit钩子里面
    ...
    def long_press_drag(self, start_widget, end_widget,
        ...
        Raises: 
            Exception: 没有找到指定控件
        """
        self.log.record_action_start("long press drag",
                                     "'{}', '{}'".format(start_widget, end_widget))
        ...
        if self.lasted_widget == start_widget:
            ...
            if self.__check_widget_exist(end_widget, region2):
                ...
                if self.virtual_debug:
                    print("[Virtual Debug] long press drag('{}', '{}'): "
                          "Failed!".format(start_widget, end_widget))
                    self.log.record_action_end()
                    return
                else:
                    self.log.record_exception()
